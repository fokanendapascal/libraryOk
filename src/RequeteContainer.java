...
public class RequeteContainer {
    ...
    public static String afficherEmpruntsRetournes() {
        return "SELECT " +
                ...
                " ELSE 'Non' " + " END AS PenaliteEffective " +
                "FROM Emprunts e " +
    ...
    //ajouter d'un livre
    public static String insertLivre( String val1, String val2, String val3, int val4) {
        return "INSERT INTO livres (titre, auteur, categorie, nombreExemplaires) VALUES ('" + val1 + "', '" + val2 + "', '" + val3 + "', '" + val4 + "')";
    ...
    // Cas pour ID (valeur numérique)
            return "UPDATE " + table + " SET " + columnModif + " = "
                    + "'"+newVal+"'" + " WHERE " + columnSearch + " = " + "'" +
                    searchVal + "'";
        } else {
    // Cas pour texte (texte insensible à la casse)
            return "UPDATE " + table + " SET " + columnModif + " = '" + newVal + "' WHERE LOWER(" + columnSearch + ") LIKE LOWER('%" +
            searchVal + "%')";
        }
    }
    ...
    //ajouter un emprunt
    public static String ajouterEmprunt(String nom, String prenom, String titre) {
        return "INSERT INTO Emprunts (membreId, livreId, dateEmprunt, dateRetourPrevue)  VALUES ( (SELECT id FROM Membres WHERE LOWER(Nom) LIKE LOWER('%" + nom + "%') AND LOWER(Prenom) LIKE LOWER('%" + prenom + "%')LIMIT 1), (SELECT id FROM Livres WHERE LOWER(titre) LIKE LOWER('%" + titre + "%')LIMIT 1), CURRENT_DATE, CURRENT_DATE + INTERVAL '14 days')";
    }
